...
\thesissetup{
    ...
    declaration = {
      ...
      During the preparation of this thesis, I used large language models (OpenAI ChatGPT, Anthropic Claude, and GitHub Copilot) for brainstorming, research discussion, development and text editing. The final content was reviewed and revised by me, and I take full responsibility for the work presented.
    },
    thanks      = {%
...
\textbf{Chapter~\ref{chap:architecture}} presents the architecture and design of the ServiceSlicer platform, detailing its components, data flows, and integration with external tools for static analysis and benchmarking.
...
\textbf{Chapter~\ref{chap:demonstration}} demonstrates ServiceSlicer through case studies on real-world monolithic applications, assessing its effectiveness in generating decomposition candidates and measuring scalability under load.
...
This chapter presents the architecture and design of the ServiceSlicer platform, building on the decomposition challenges and methodologies discussed previously. It describes the system's structure, key components, and data flows, explaining how ServiceSlicer integrates static analysis and empirical benchmarking to support monolith decomposition. The following sections provide a detailed overview of each architectural element and their roles in enabling reproducible evaluation of candidate microservice boundaries.
...
\begin{figure}[htb]
  ...
  \caption{Detailed diagram of the two workflows supported in ServiceSlicer}%
  \label{fig:context}
\end{figure}
...
Several practical limitations emerged during the Technika demonstration that inform future development priorities:

\begin{itemize}
  \item \textbf{Configuration coupling}: Modifying existing operational settings affects already completed or running benchmarks. The current implementation lacks sufficient separation between configuration snapshots and benchmark runs, requiring careful version management of operational profiles.

  \item \textbf{Limited algorithm configurability}: The graph clustering algorithms expose minimal configuration options. More advanced parameterization—such as resolution settings for modularity optimization or seed control for Label Propagation—would enable finer-grained exploration of decomposition alternatives.

  \item \textbf{Behavior model sensitivity}: Load testing revealed a high rate of failing requests due to improper variable bindings in behavior models. For example, \texttt{POST /transactions} returned numerous 400 errors because of misconfigured request bodies. This highlights the importance of thorough validation runs and the need for improved debugging support in the operational setting configuration interface.
  \item \text{Scalability gap and performance offset visualization}: The current reporting interface lacks intuitive visualizations for interpreting some of the metrics. Graphical representations of scalability gaps and performance offsets across load levels would enhance result interpretability.
\end{itemize}

These limitations reflect the prototype nature of the current implementation and identify concrete areas for engineering refinement in future development cycles.

\subsection{Scope and Representativeness}
...
The demonstrations presented in this chapter validate that ServiceSlicer can successfully process real applications, generate meaningful decomposition candidates, and execute automated benchmarks against containerized deployments. Both RealWorld Conduit and Technika progressed through the complete workflow—from static dependency extraction through algorithmic clustering to multi-level scalability assessment—producing actionable insights for architectural decision-making.

Key observations emerged across both systems. Static analysis consistently identified structurally cohesive clusters, though these required interpretation through domain knowledge to yield practical service boundaries. The actor-aligned decomposition approach proved effective for the industrial system, demonstrating that organizing services around user personas can balance architectural clarity with operational feasibility. Operational profile construction required substantial domain expertise, highlighting the importance of realistic workload modeling for meaningful scalability assessment.

The practical limitations encountered—configuration coupling, algorithm configurability constraints, and behavior model sensitivity—identify concrete areas for future engineering refinement and strengthen the platform's applicability to a broader range of decomposition scenarios.

\chapter{Conclusions \& Future Directions}\label{chap:conclusions}
